...
DescribeRemote.prototype.fetchPersonAccountRecordTypes = function(cb) {
	...
	self._conn.query("SELECT Id, DeveloperName, SobjectType, IsPersonType FROM RecordType WHERE SobjectType='Account' AND IsPersonType=True", function(err, result) {
...
DescribeRemote.prototype.fetchFlowDefinitions = function(cb) {
	...
	self._conn.tooling.query("SELECT Id, DeveloperName, ActiveVersion.VersionNumber FROM FlowDefinition", function(err, result) {
		// don't fail on error
		if (err) {
...
DescribeRemote.prototype.fetchStandardPicklists = function(cb) {
	...
	self._conn.tooling.query("SELECT Id, EntityDefinition.QualifiedApiName, QualifiedApiName FROM FieldDefinition WHERE EntityDefinition.QualifiedApiName IN (" + values.join(',') + ") AND DataType = 'Picklist'", function(err, result) {
		// don't fail on error
		if (err) {
			self.warnings.push('Warning: ' + err);
			return cb(null, []);
